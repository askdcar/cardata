...
const [filteredCars, setFilteredCars] = useState([]);
  ...
  const parseNested = (obj) => {
    ...
    if (typeof obj === "string") {
      ...
      catch (e) { console.warn("Failed to parse nested JSON:", obj); return {}; }
    }
    return obj;
  };
  ...
  // Update filteredCars whenever cars or searchTerm changes
useEffect(() => {
  const term = searchTerm.toLowerCase();
  setFilteredCars(
    cars.filter(car =>
      car.name.toLowerCase().includes(term) ||
      (car.brand?.name || car.brandName || "").toLowerCase().includes(term) ||
  ...
  const handleChange = (e, nestedKey = null) => {
    ...
    if (inputType === "checkbox" || inputType === "switch") {
      if (nestedKey) {
        setFormData(prev => ({ ...prev, [nestedKey]: { ...prev[nestedKey], [name]: !!checked } }));
      } else {
  ...
  const removeVariant = index => { const updated = [...formData.variants]; updated.splice(index,1); setFormData(prev => ({ ...prev, variants: updated })); };
  const handleVariantChange = (index, field, value) => { const updated = [...formData.variants]; updated[index][field] = value; setFormData(prev => ({ ...prev, variants: updated })); };
  const handleVariantImageChange = (index, files) => { const updated = [...formData.variants]; updated[index].images = Array.from(files); setFormData(prev => ({ ...prev, variants: updated })); };
  ...
  const openEdit = car => {
    ...
    setFormData({
      ...
      engineSpec: { ...initialState.engineSpec, ...(parseNested(car.engineSpec)) },
      fuelPerformance: { ...initialState.fuelPerformance, ...(parseNested(car.fuelPerformance)) },
      suspensionSteering: { ...initialState.suspensionSteering, ...(parseNested(car.suspensionSteering)) },
      dimensionsCapacity: { ...initialState.dimensionsCapacity, ...(parseNested(car.dimensionsCapacity)) },
      comfortConvenience: { ...initialState.comfortConvenience, ...(parseNested(car.comfortConvenience)) },
      interiorFeatures: { ...initialState.interiorFeatures, ...(parseNested(car.interiorFeatures)) },
      exteriorFeatures: { ...initialState.exteriorFeatures, ...(parseNested(car.exteriorFeatures)) },
      safetyFeatures: { ...initialState.safetyFeatures, ...(parseNested(car.safetyFeatures)) },
      entertainment: { ...initialState.entertainment, ...(parseNested(car.entertainment)) },
      adasFeatures: { ...initialState.adasFeatures, ...(parseNested(car.adasFeatures)) },
      internetFeatures: { ...initialState.internetFeatures, ...(parseNested(car.internetFeatures)) },
      seo: car.seo || initialState.seo,
...
<div className="mb-3">
  <input
    ...
    placeholder="Search by Name, Brand, Fuel Type, or Category"
    value={searchTerm}
      ...
      <Modal show={showModal} onHide={()=>setShowModal(false)} size="xl" scrollable>
        ...
        <Modal.Body>
          <Form>
            ...
            <Row className="mb-3">
              <Col md={3}><Form.Check type="switch" label="Upcoming" name="isUpcoming" checked={formData.isUpcoming} onChange={handleChange}/></Col>
              <Col md={3}><Form.Check type="switch" label="Popular" name="isPopular" checked={formData.isPopular} onChange={handleChange}/></Col>
              <Col md={3}><Form.Check type="switch" label="Latest" name="isLatest" checked={formData.isLatest} onChange={handleChange}/></Col>
              <Col md={3}><Form.Check type="switch" label="Electric" name="isElectric" checked={formData.isElectric} onChange={handleChange}/></Col>
            </Row>
            ...
            {/* SEO Fields */}
            <h5 className="mt-3">SEO Details</h5>
            <Row>
              <Col md={6}>
                <Form.Group className="mb-2">
                  <Form.Label>Meta Title</Form.Label>
                  <Form.Control type="text" name="metaTitle" value={formData.seo.metaTitle} onChange={(e)=>handleChange(e,"seo")}/>
                </Form.Group>
              </Col>
              <Col md={6}>
                <Form.Group className="mb-2">
                  <Form.Label>Meta Description</Form.Label>
                  <Form.Control type="text" name="metaDescription" value={formData.seo.metaDescription} onChange={(e)=>handleChange(e,"seo")}/>
                </Form.Group>
              </Col>
            </Row>
            <Row>
              <Col md={6}>
                <Form.Group className="mb-2">
                  ...
                  <Form.Control type="text" name="metaKeywords" value={formData.seo.metaKeywords} onChange={(e)=>handleChange(e,"seo")}/>
                </Form.Group>
              </Col>
              <Col md={6}>
                <Form.Group className="mb-2">
                  <Form.Label>OG Title</Form.Label>
                  <Form.Control type="text" name="ogTitle" value={formData.seo.ogTitle} onChange={(e)=>handleChange(e,"seo")}/>
                </Form.Group>
              </Col>
            </Row>
            <Row>
              <Col md={12}>
                <Form.Group className="mb-2">
                  ...
                  <Form.Control as="textarea" rows={2} name="ogDescription" value={formData.seo.ogDescription} onChange={(e)=>handleChange(e,"seo")}/>
                </Form.Group>
              </Col>
            </Row>
...
{renderNestedView("Internet Features", parseNested(viewCar.specs?.internetFeatures))}
   ...
   {viewCar.seo && (
                <>
                  <h5>SEO Details</h5>
                  <p><strong>Meta Title:</strong> {viewCar.seo.metaTitle}</p>
                  <p><strong>Meta Description:</strong> {viewCar.seo.metaDescription}</p>
                  <p><strong>Meta Keywords:</strong> {viewCar.seo.metaKeywords}</p>
                  <p><strong>OG Title:</strong> {viewCar.seo.ogTitle}</p>
                  <p><strong>OG Description:</strong> {viewCar.seo.ogDescription}</p>
                </>
              )}
